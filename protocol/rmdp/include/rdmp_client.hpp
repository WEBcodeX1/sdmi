...
// Workflow
// --------
//  1. addNewTask(msg) – writes the task to the shared backend and enqueues a
//     burst of UDP multicast announcements.
//  2. runOnce()       – called in a tight loop by the application; it:
//       a. drains the pending-multicast burst queue (sends UDP datagrams),
//       b. polls the backend for tasks added by *other* client instances
...
private:
    ...
    // UUIDs already discovered (either created locally or found via backend).
    ...
    // Time (ms) of the last backend poll.
    ...
    // Poll backend for task UUIDs not yet in known_tasks_ and enqueue bursts.
    void pollS3ForNewTasks();

    // Persist a new task to the backend.
